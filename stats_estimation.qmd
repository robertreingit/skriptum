...
Damit sind jetzt die wichtigsten Komponenten von statistischen Analysen, zumindest so wie sie in den meisten Fällen in der Literatur angewendet werden, erarbeitet. Hypothesentestung, p-Werte, Konfidenzintervalle, Typ-I und Typ-II Fehler und Konfidenzintervalle. Tatsächlich sind diese verschiednen Konzepte einer Mischung aus zwei Ansätzen die im Laufe der Zeit miteinander verschmolzen wurden und zum Teil auf widersprüchlichen Ansätzen beruhen. Ein Großteil der in der Statistik verwendet Methoden geht auf die Arbeiten von Ronald A. Fisher (1890-1962) zurück. Unter Fishers statistschen System war die Nullhypothese $H_0$ zentral und die Ablehnung derer basierend auf dem dazugehörenden p-Wert. Aus Fishers Ansatz erfolgte auch die Konzepte der $H_0$ Hypöothese als diejenige unter der *nichts* passiert. Unter Fisher waren die Konzepte Typ-I und Typ-II Fehler nicht vorhanden. Dieser Ansatz wurde unter dem Begriff *inductive inference* zusammengefasst (@fisher1935). D.h. auf der Basis von Daten soll eine Aussage über die Realität unter der Einschätzung der Unsicherheit gemacht werden (@lehmann1993). Zu diesem Zweck wird eine $H_0$-Hypothese aufgestellt und diese wird anhand der Daten mittels eines p-Wert beurteilt. Der p-Wert wird unter Fisher als Evidence für oder gegen die $H_0$ Hypothese angesehen. Aus Fishers inductive inference kommt auch die Begrifflichkeit der *Signifikanz*. Induktiv deswegen, weil von den Daten auf die Allgemeinheit geschlossen wird und dieser Schritt ist immer mit Unsicherheit behaftet.

Dem Gegenüber steht der Ansatz von Nullhypothese $H_0$ und einer Altervativhypothese $H_1$. Dieser Ansatz geht auf die Arbeiten der Statistiker Jerzy Neyman (1894-1981) und Egon S. Pearson (1895-1980) zurück. Unter diesem System ist die Minimierung eines Entscheidungsfehler der zentrale Ansatzpunkt. Unter der Betrachtung von vielen Wiederholung, wie können Entscheidungen getroffen werden die im Mittel die Anzahl der Fehler minimieren. Aus diesem Ansatz haben Pearson-Neyman die Konzepte Typ-I und Typ-II und die Power entwickelt (@lehmann1993). Unter diesem System haben der p-Wert und Konfidenzintervalle keine Bedeutung sondern es geht lediglich um die Entscheidung (provisorisch) die $H_0$ anzunehmen oder die $H_0$ abzulehnen um über **viele Wiederholungen des Experiments** hinweg optimale Entscheidungen zu treffen. Der Hintergrund vor dem Neyman und Pearson die Theorie entwickelt haben, ist stärker durch einen industriellen Kontext geprägt um zum Beispiel bei einem Fließband sicher zu stellen, dass die Qualität der Produkte einem bestimmten Standard entspricht. In diesem Zusmmenhang ist die *Wiederholung* von Experimenten, die zufällige Auswahl von Objekten, dementsprechend einfacher nachvollziehbar. Unter dem System von Pearson-Neyman ist die Durchführung eines Experiments dabei ohne die explizite Berechnung der Power und folglich der expliziten Aufstellung einer $H_1$ Hypothese bedeutungslos. Pearson-Neyman haben diesen Ansatz unter dem Begriff *inductive behavior* zusammengefasst. Die Signifikanz ist weniger von Bedeutung, sondern nur welche Entscheidung getroffen wird und wie die Anzahl der Fehlentscheidungen minimiert werden können.

D.h. unter Fisher ist der Begriff der Evidenz, der p-Wert, zentral, während unter Neyman-Pearson der Begriff des Fehler, $\alpha$, zentral ist. Damit verbunden ist auch beispielsweise, das das Konzept der Power unter Fischers Signifikanztest keine Rolle spielt, da keine $H_1$ vorhanden ist. Es lässt sich zeigen, dass diese beiden Ansätze auf Basis der gleichen Daten zu unterschiedlichen Aussagen kommen können, d.h. im Widerspruch zueinander stehen. Die Unterschiede in der Auffassung wie Daten statistisch Bewertet werden hat zu einer regelrechten Fehde vor allem zwischen Fisher und Pearson geführt. Unglücklicherweise sind im Laufe der Jahre die beiden Ansätze bei Anwendern der Statistik immer mehr verschmolzen und haben dazu geführt, dass ein Großteil der wissenschaftlichen Analysen mit einem in sich widersprüchlichen statistischen System durchgeführt wird (@hubbard2003, @hager2013, @gigerenzer2004). Ironischerweise sind beide Gruppen sich einig, dass das Signifikanzlevel bzw. der Fehlerlevel mit $5$% nicht sakrosankt ist sondern entsprechend der Fragestellung angepasst werden sollte (@lehmann1993).

Die Vermischung der beiden Konzepte hat auch dazu geführt, dass der p-Wert und der $\alpha$-Level miteinander verwechselt werden. Der p-Wert ist eine Aussage über die Daten unter der Annahme der $H_0$. Das $\alpha$-Level ist dagegen eine Aussage über die Fehlerrate. Die Fehlerrate macht inhaltlich nur Sinn bei einer Betrachtung über viele Experimente hinweg. Bei einem einzelnen Experiment wird entweder ein Fehler oder eben nicht gemacht. Ein p-Wert von $p = 0.03$ ist keine Fehlerrate, sondern eine Aussage darüber mit welcher Wahrscheinlichkeit ein beobachteter oder ein extremerer Wert unter der $H_0$ auftritt. In dieser Aussage taucht das Konzept eines Fehlers nicht auf.Tatsächlich ist die Missinterpretation nicht nur unter Anwenderinnen sondern auch unter Statiskerinnen weit verbreitet (@hubbard2003).
