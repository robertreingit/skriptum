...
Die Matrizen müssen für die Matrixmultiplikation konform sein, was in diesem Fall für $\mathbf{A}$ und $\mathbf{B}$ für beide Richtung der Fall ist.

```{r}
B %*% A
```

Hier wird auch wieder deutlich, dass die Matrizenmultiplikation nicht kommutativ ist. Die elementweise Multiplikation funktioniert nur in dem Fall, wenn beide Matrizen die gleichen Dimensionen haben.
...
::: {#exm-matrix-sym-inv}
### Inverse Matrix eines symmetrischen Matrix $\mathbf{A}$

Es sei die folgende symmetrische Matrix gegeben:
