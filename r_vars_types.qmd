...
Nochmal zur Wiederholung, eine Matrix der Dimension $m \times n$ wird in der Mathematik wie folgt definiert.
...
Mit der Funktion `diag()` kann eine quadratische Diagonalmatrix \index{Diagonalmatrize} erzeugt werden. Eine Diagonalmatrix hat nur auf der sogenannten Hauptdiagonalen Einträge, während alle anderen Werte $=0$ sind (siehe Formel \eqref{eq-r-kickoff-diag}).
...
Soll eine Diagonalmatrix erstellen die bestimmte Wert auf der Hauptdiagonalen hat, dann kann mittels `diag()` auch einen Vektor mit den gewünschten Werten übergeben.

```{r}
diag(1:4) 
```

Wenn `diag()` dagegen eine Matrix übergeben wird, dann werden die Elemente auf der Hauptdiagonalen extrahiert.
...
Eine Matrix kann mit Hilfe der `t()` Funktion transponiert werden.
...
Wie im Anhang besprochen, ist die Berechnung der Inversen $\mathbf{A}^{-1}$ einer quadratischen Matrix $\mathbf{A}$, wenn sie existiert, ein zentrales Problem in der Linearen Algebra. In `R` kann die Inverse $\mathbf{A}^{-1}$ einer Matrix numerisch mittels der Funktion `solve()` berechnet werden.
...
Um beispielsweise das Gleichungssystem:
...
D.h. die zu invertierende Matrix $\mathbf{A}$ und der Vektor $\mathbf{y}$ werden, direkt zusammen an `solve()` übergeben. In diesem Fall wird die Inverse tatsächlich gar nicht berechnet sondern `solve()` mit sich interne Optimierungen zunutze, die diesen Schritt überflüssig machen. Diese ist in den meisten Fällen schneller und numerisch stabiler, insbesondere bei sehr großen Gleichungssystemen. Weitere Funktionen zur Lösung von Gleichungssystemen die numerisch vorteilhaft sind, wie beispielweise die QR-Zerlegung sind in der Dokumentation zu finden.
