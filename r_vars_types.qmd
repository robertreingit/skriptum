...
::: {#exm-matrix-sym-inv}
### Inverse Matrix einer symmetrischen Matrix $\mathbf{A}$
...
```{r}
F <- matrix(1:12, nr = 4, nc = 3) # <1>
F
apply(F, 1, sum) # <2>
```
1. Eine Matrix $F$ mit vier Zeilen und drei Spalten erstellt.
...
apply(F, 2, sum) # <1>
```
1. Die Summenfunktion wird entlang der zweiten Dimension (Spalten) auf $F$ angewendet. 
