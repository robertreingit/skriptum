...
f_A <- c("A","B","C")
beta_to_mu <- codingMatrices::mean_contrasts(contr.treatment(f_A))
...
\begin{equation*}
\beta_i = \mu_i - \bar{\mu}, i = 2,3,\ldots,K
\end{equation*}
...
Und tatsächlich, ist dies der gesuchte Wert.

Insgesamt heißt dies, das wir um die Koeffizienten interpretieren zu können, immer wissen müssen welches Kodierungssystem verwendet wird. Leider führen unterschiedliche Kodierungssysteme zu relativ subtilen Unterschieden die ganz und gar nicht offensichtlich sind. Vor allem später, wenn wir uns mit faktoriellen experimentellen Designs beschäftigen wird dies noch mal stärker von Relevanz werden. Diese kurze Einführung basiert hauptsächlich auf @venables2023. Dies ist auch die primäre Quelle um sich in die Thematik noch einmal tiefer einzulesen. 
