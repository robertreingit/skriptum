...
## Herleitung der Normalengleichungen (Fortgeschritten) {#sec-step-by-step-normal}
...
\begin{align*}
 \sum(x_i-\bar{x})(y_i-\bar{y}) &= \sum (x_iy_i-\bar{x}y_i-x_i\bar{y}+\bar{x}\bar{y})  \\
 &= \sum x_i y_i - \sum\bar{x}y_i - \sum x_i \bar{y} + \sum \bar{x} \bar{y}  \\
...
\begin{align*}
\frac{\partial \sum (y_i - \beta_0 - \beta_1 x_i)^2}{\partial \beta_0} &= \sum\frac{\partial}{\partial \beta_0}(y_i - \beta_0- \beta_1 x_i)^2  \\
...
\begin{align*}
\frac{\partial \sum (y_i - \beta_0 - \beta_1x_i)^2}{\partial \beta_1} &= \sum\frac{\partial}{\partial b}(y_i - \beta_0 - \beta_1x_i)^2  \\
...
\Leftrightarrow\mkern40mu && \sum (x_i-\bar{x})(y_i-\bar{y}) = \beta_1 \left(\sum x_i^2 - n\bar{x}^2\right) \nonumber \\
