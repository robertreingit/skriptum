...
Eine weitere Variante wiederholt Anweisungen durchzuführen, bietet das `while`-Schleifenkonstrukt.
...
Der Schleifeninhalt wird bei der `while`-Schleife so lange wiederholt, bis die Kondition `FALSE` wird. Ein Beispiel.
...
x <- 10
while (x < 15) {
  cat("x ist kleiner 15.\n")
  x <- x + 1
}
```

Bei der `while`-Schleife muss selbst darauf geachtet werden, dass die Bedingung zum Austritt aus der Schleife selbst gesetzt wird, ansonsten läuft die Schleife immer weiter und es kommt zu einer **Endlosschleife**. Oft wird dies mit Hilfes eines Sicherheitstest in Form eine expliziten Schleifenzählers kontrolliert.
...
Die `while`-Schleife ist dann sinnvoll, wenn nicht von vorne herein bekannt ist, wie oft eine Anweisung durchgeführt werden soll. Insgesamt ist die `while`-Schleife flexibler aber dadurch auch fehleranfälliger in der Programmierung. In vielen Fällen kann ein gleichwertiges Konstrukt mittels einer `for`-Schleife erstellt werden, was dann auch oft leichter zu verstehen ist. Als Merksatz gilt oft, wenn klar ist wie oft etwas durchgeführt werden soll, dann sollte eine `for`-Schleife verwendet werden, wenn nur klar ist wann es aufhören soll, dann eine `while`-Schleife.
