...
::: {#exm-r-flow-02}
Sei der folgende Anwendungsfall gegeben. Wir haben Daten von drei Sprungen bezüglich der vertikalen Höhe des COM und wollen aus den drei Kurven jeweils die maximale Höhe extrahieren. Seien die folgenden drei Kurven gegeben die wir alle in einer Liste hinterlegt haben.
...
Wir verwenden hier nur sehr "übersichtliche" Kurven um uns auf das Programm konzentrieren zu können. Da die Signale in einer Liste hinterlegt sind, können wir durch die einzelnen Elemente mittels dese `[[]]`-Operator iterieren. Wir definieren uns zunächst einen Vektor der die Maximalhöhen aufnimmt die wir mittels der `max()`-Funktion bestimmen. Mit einer `for`-Schleife führen wir die Iteration durch.
