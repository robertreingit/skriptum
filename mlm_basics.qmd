...
Es lässt sich zeigen, dass die Varianz der geschätzten Modellkoefizienten $b_j$ die folgende Form hat (siehe Formel \eqref{eq-mlm-basics-varb_j}:
...
Der zweiter Term in Formel \eqref{eq-mlm-basics-varb_j} $\frac{1}{1-R_j^2}$ wird als Variance Inflation Factor (VIF) \index{Variance Inflation Factor} bezeichnet. Wie der Name bereits andeutet, kommt es durch die Korrelation der Prädiktorvariablen untereinander es zu einer *Inflation der Varianzen*. D.h. die Standardfehler, die Wurzeln der jeweiligen Varianzen, der Modellparameter werden vergrößert was dazu führen kann, dass die Modellkoeffizienten keine statistische Signifikanz aufweisen obwohl ein klarer Zusammenhang zwischen der Prädiktorvariablen und der abhängigen Variablen besteht. Anhand \eqref{eq-mlm-basics-varb_j} ist zu erkennen, dass wenn der Determinantionskoeffizient, also die Korrelation der Prädiktorvariablen größer Wert, der zweite Term ebenfalls an Größe zunimmt und dementsprechend die Varianz sich vergrößert.

Der VIF bietet daher eine einfache Möglichkeit, zumindest heuristisch, das gefittete Regressionsmodell auf Multikollinearitäten zu überprüfen. Der VIF wird für jede Prädiktorvariable bestimmt. Bei der Interpretatation der VIF gibt es keinen *magischen* Wert der für Probleme spricht. Im Allgemeinen werden die VIFs niemals komplett null sein. Bei der Beurteilung von Problemen auf Grund von Multikollinearität wird üblicherweise der größte VIF Wert im Modell betrachtet.
...
## Unter der Motorhaube von `lm()` die Modellmatrix

Für die weitere Arbeit mit der multiplen linearen Regression ist es von Vorteil ein etwas tiefergehendes Verständnis der Berechnungen in `R` bei der Verwendung von `lm()` zu haben. Insbesondere die Kenntnis der sogenannten Modellmatrix ist dabei von Vorteil. 

::: {#def-model-matrix}
## Modellmatrix 

Die Modellmatrix \index{Modellmatrix}, auch als Designmatrix \index{Designmatrix} oder Regressionsmatrix \index{Regressionsmatrix}, ist eine Matrix welche die zur Modellierung verwendeten Daten in Form einer Matrix enthält. Jede Zeile der Modellmatrix repräsentiert eine Beobachtung, während jede Spalte einer Prädiktorvariable entspricht. Die Modellmatrix wird oft mit dem Buchstaben $X$ bezeichnet.

:::

In `R` kann eine Modellmatrix mit Hilfe der Funktion `model.matrix()` erzeugt werden. Am einfachsten ist die Funktion anhand eines Beispiels nach zu vollziehen. Sei der folgende einfache Beispieldatensatz gegeben:
...
Wenn nun eine lineares Modell an die *Daten* gefittet wird, dann würde die folgende Syntax verwendet werden `lm(y ~ x_1 + x_2)` entsprechend zu dem Modell $y_i = \beta_0 + \beta_1 x_{1i}+ \beta_2 x_{2i} + \epsilon_i$. Was nun intern in `lm()` passiert ist, das die Berechnung auf ein mathematisches Problem abgebildet wird, das mit Hilfe von Matrizenalgebra gelöst wird. Die genaueren Schritte sind zunächst nicht weiter wichtig, sondern nur der erste Schritt die Erstellung der Modellmatrix $X$. Die erzeugte Modellmatrix kann angezeigt werden, indem die Funktion `model.matrix()` auf das Modellobjekt angewendet wird.
...
Alternativ kann die Modellmatrix anhand der *Formel* und dem Datensatz erstellt werden:

```{r}
#| echo: true

model.matrix(~ x_1 + x_2, data = df)
```

Laut der Definition, ist die Modellmatrix $X$ die numerische Darstellung der Prädiktorvariablen mittels einer Matrix. In der zweiten Spalte sind die Werte der Variablen `x_1`, während in der dritten Spalte die Werte der Variablen `x_2` eingetragen sind. Die Spalten entsprechen also den jeweiligen Einträgen der Prädiktorvariablen $x_{ji}$. Die erste Spalte sticht hier etwas raus, da hier nur $1$ eingetragen sind. Diese Spalte ist dem ersten Modellkoeffizienten $\beta_0$ zugeordnet, also dem *konstanten* $y$-Achsenabschnitt. Dies hat den Vorteil, dass die vorhergesagten Werte durch eine einfache Matrixmultiplikation berechnet werden können (im Anhang ist dazu eine kurze Erläuterung, ist aber nicht notwendig für das weitere Verständnis).
