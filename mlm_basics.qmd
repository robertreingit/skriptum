...
Wieder wird der Wert für den Steigungskoeffizienten $\beta_2$ aus dem Modell mit beiden Variablen der multiplen Regression erhalten. Das gleiche Prinzip generalisiert auch für mehr als zwei Variablen, nur das in diesem Fall nicht mehr einer Regression der Variable $X_i$ gegen eine andere $X_j$ Variable berechnet wird, sondern es wirde eine multiple Regression von $X_i$ gegen alle anderen Prädiktorvariablen $X_j, j = 1, i-1,i+1,K$ berechnet.
