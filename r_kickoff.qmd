...
Vektoren sind grundlegende Datentypen in `R` und können sowohl numerische als auch nicht-numerische Werte speichern. Allerdings kann immer nur ein Typ von Werten in einem Vektor gespeichert werden. Da Vektoren eine typische Art wie Daten organisiert sind, werden sien entsprechend in `R` unterstützt. Vektoren können als eine geordnete Sammlung von Elementen betrachtet werden. 

Die Hauptart und Weise wie Vektoren in `R` erstellt werden, ist mit der Funktion `c()` \index{c()} von combine. Wollen wir zum Beispiel einen numerischen Vektor mit den Zahlen

\begin{equation}
v_1 = \begin{pmatrix}3\\7\\8\end{pmatrix}
\end{equation}

erstellen, dann sieht das in `R` vollgendermaßen aus.
...
Wollen wir einen Vektor mit Zeichenketten erstellen, verwenden wir entsprechend.
...
Wir können auf einzelne Elemente oder Teil eines Vektor zugriffen mittels des sogenannten subsettings zugreifen. Dazu zunächst einmal hat jeder Vektor eine Länge die wir mit `length()` bestimmen können.

```{r}
length(v_2)
```

Mit eckigen Klammern `[]` können wir Teilelemente des Vektors extrahieren. Zum Beispiel das zweite Element von $v_1$.

```{r}
v_1[2]
```

In `R` kann auch wiederum ein Vektor zum subsetten übergeben werden, beispielweise um das 1. und 3. Element aus $v_2$ zu extrahieren.

```{r}
v_2[c(1,3)]
```

Der zurückgegebene Typ ist in diesem Fall auch wieder ein Vektor. Wir können dadurch zum Beispiel einen Vektor erstellen der länger als der Ursprungsvektor ist.

```{r}
v_1[c(1,1,2,2,3,3)]
```

Zum subsetten können auch logische Vektoren verwendet werden. Der resultierende Vektor ist ein Vektor mit denjenigen Element, für der logische Vektor `TRUE` ist.
...
Numerische Vektoren verstehen als mathematischen Operatoren, diese werden jedoch elementweise ausgeführt. Als zum Beispiel die Multiplikation von $v_1$ mit Skalar $3$ führt zu.
...
Eine etwas unglückliche gewählte Eigenschaft von `R`, dass auch Vektoren mit unterschiedlichen Längen addiert (subtrahiert, multipliziert, dividiert) werden können, allerdings nur wenn die Länge des längeren Vektors eine Vielfaches des kürzeren ist.
...
Dieses Verhalten ist wie gesagt etwas unglücklich gewählt, das sich schnell subtile Fehler in den Code einschleichen können, die dann nur sehr schwer zu identifzieren sind. Daher sollte auf diese Feature möglichst verzichtet werden.

Per default werden Vektoren in `R` nicht als Spaltenvektoren wie in der Mathematik angesehen. Das kann manchmal zu zu Problemen in Rechnungen führen, wenn die Rechnung von der üblichen Algebra bei Vektoren und Matrizen ausgeht. Dies sollte daher im Hinterkopf behalten werden.

Wenn das Skalarprodukt $\|v_x v_y\|^2 = \sum_{i=1}^nv_{xi}v_{yi}$ zweier Vektoren berechnet werden soll, dann muss ein spezieller Matrizenmultiplikator benutzt werden `%*%`.

```{r}
v_3 %*% v_4
```

In `R` gibt es verschiedene Funktion um schnell Vektoren mit bestimmten Strukturen zu erstellen. Wir ein Vektor mit aufeinanderfolgenden Ganzzahlen benötigt kann der `:` Operator verwendet werden. Es wird ein Vektor nach dem Muster `a:b` = $(a,a+1,\ldots,b-1,b)$ erstellt.
...
Soll die Zahlenfolgen nicht ganzzahlig sein sondern andere Intervalle haben, dannn kann die `seq()` Funktion verwendet werden.
...
Wie in dem Beispiel ersichtlich, kann so schnell beliebige Wiederholungen erzeugt werden.

Eine praktische Funktion im Zusammenhang mit Vektoren ist wiederum die Funktion `paste()` die wir im Zusammenhang mit Zeichenketten kennengelernt haben. Werden Vektoren an `paste()` übergeben, werden die kürzeren Werte so oft wiederholt bis die Länge des längsten Vektors erreicht wird und dann erfolgt die Zusammensetzung. Daher wird dann ein Vektor zurückgegeben.
