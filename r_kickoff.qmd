...
$$
muss wiederum der schon bekannte Operator `%*%` verwendet werden. Die Matrizen müssen dazu konform für die Mutliplikation sein.
...
Um die Dimension einer Matrize zu bestimmen gibt es in `R` die Funktion `dim()`.

```{r}
dim(A)
```
Oder auch die beiden Kurzfunktionen:
...
Mit der Funktion `diag()` kann eine Diagonalmatrize erstellt werden.
...
diag(1:4) # <2>
```
1. Wenn ein Skalar $a$ übergeben wird, die wird die Einheitsmatrize der Dimension $a$ erstellt.
2. Wenn ein Vektor übergeben wird, dann eine Diagonalmatrize mit den Werten des Vektors.

Wenn `diag()` eine Matrize übergeben wird, dann werden die Elemente auf der Diagonalen extrahiert.
