...
Um bei unserem Experiment sicher zu stellen, dass wir auch eine realistische Chance haben einen gewünschten Effekt zu finden, müssen wir vor dem Experiment eine Power-Analyse durchführen um die notwendig Stichprobengröße zu bestimmen.
...
Unter der $H_1$ folgt $F$ einer $F(df_1, df_2, \delta^2)$-Verteilung. $\delta^2$ ist der Nichtzentralitätsparameter (oft auch $\lambda$). Diese Verteilung wird daher auch als nichtzentrale $F$-Verteilung bezeichnet. Der Nichtzentralitätsparameter hängt dabei mit unserem Wert $Q(\tau_i)$ zusammen. Umso größer der Unterschied zwischen den verschiedenen Faktorstufen, umso größer auch der Nichtzentralitätsparameter. D.h. wir benötigen Informationen über die Effekte zwischen den Stufen der Konditionen um $\lambda$ zu bestimmen und dann anhand des Nichtzentralitätsparmeter die korrekte Verteilung unter der $H_1$ zu bestimmen.

Im Fall 1. gehen versuchen wir die notwendigen Informationen anhand bereits dokumentierter Ergebnisse in der Literatur zu bestimmen. Beispielsweise anhand der Quadratsummen aus den $F$-Tabellen oder im einfachsten Fall direkt mittels der dokumentierten Effektstärke $f$.

Konkret berechnet sich $\delta^2$ nach.
...
Die ermittelte Effektstärke $f$ kann dann verwendet werden um die Stichprobengröße zu ermitteln.

### CRD Stichprobengröße a-priori in `R` ermitteln

In `R` können wir direkt die Effektstärke $f$ verwenden mittels der Funktion `pwr.anova.test()` aus dem package `pwr`. Bei der Funktion muss immer eins von $f$, $k$, $\alpha$, $power$ oder $n$ nicht übergeben werden und die Funktion bestimmt denn fehlenden Wert. Wollen wir z.B. die notwendige Stichprobengröße pro Faktorstufe für $f = 0.39, K = 3, \alpha = 0.05, \text{power} = 0.8$ bestimmen, dann können wir dies wie folgt erreichen.
