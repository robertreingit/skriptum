...
In @fig-sts-sig-dist-sigma ist zu sehen, dass beide Verteilungen ihren Mittelpunkt an der gleichen Stelle haben, aber die rote Verteilung mit $\sigma_1^2=2$ breiter ist als die andere Verteilung. Dies bedeutet das die Werte in der Verteilung stärker um den Mittelwert herum streuen.

Die Varianz $\sigma^2$ ist wiederum ein Parameter der Verteilung, da sie die Form der Verteilung bestimmt. D.h. wenn wir wieder unsere Schreibweise von eben verwenden und $f$ die Form der Verteilung bestimmt dann gilt $f(\sigma^2)$ oder eben zusammen mit dem Mittelwert $\mu$, $f(\mu, \sigma^2)$.

Wenn aus der Varianz $\sigma^2$ die Wurzel gezogen wird, dann wird der resultierende Wert $\sigma$ als Standardabweichung bezeichnet. In der Stichprobe wird die Standardabweichung $s$ mittels der folgenden Formel berechnet
...
D.h. die Standardabweichung ist die mittlere quadrierte Abweichung vom Mittelwert (siehe Formel \eqref{eq-std}). In den meisten Fällen wird die Standardabweichung verwendet um die Streuung der Daten zu dokumentieren und nicht die Varianz. Die Standardabweichung hat den Vorteil das sie die gleiche Einheit hat wie der Mittelwert. Da die Abweichungen quadriert werden, also die quadrierten Einheiten haben, hat die Standardabweichung $s$ die gleiche Einheit wie der Mittelwert $\bar{x}$, während die Varianz der Stichprobe $s^2$ eben die quadrierten Einheiten hat.
