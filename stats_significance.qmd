...
Hier sehen wir, dass die Position der Verteilungen gleich geblieben ist. Dies sollte nicht weiter verwundern, da immer noch die beiden Hypothesen $H_0: \Delta = 0$N und $H_1: \Delta = 500$N miteinander vergleichen werden. Aber durch die Erhöhung der Stichprobengröße ist die Streuung der $D$s unter beiden Hypothesen kleiner geworden. Dies führt dazu, dass die Verteilungen steiler sind und sich dementsprechend weniger stark überlappen. Die Streuung der Statistik wird wie schon oben als Standardfehler bezeichnet.

Der Standardfehler ist **nicht** gleich der Standardabweichung in der Population bzw. der Stichprobe (siehe @fig-stats-sig-standard-error-deriv).
...
ggplot(df, aes(x,y)) +
  ...
  annotate("text", x = 1.8, y = .4,
           label=expression(paste("Standarfehler ",s[e])), size=5) +
...
Der folgende Abschnitt betrachtet wie sich die die durchschnittliche Power auf die Literatur insgesamt auswirkt. Sei eine bestimmte Anzahl von Studien bzw. Veröffentlichungen gegeben. Ein Teil dieser Studien wird signifikante Ergebnisse dokumentieren während der andere Teil keine signifikanten Ergebnisse gefunden hat. Innerhalb der signifikanten Ergebnisse wird ein Teil der Ergebnisse auf einem tatsächlichen Effekt beruhen während der andere Teil zwar statistisch signifikant ist aber eigentlich kein Effekt vorliegt. Sondern es wurde ein Typ-I Fehler begangen. Der Anteil der Studien bei denen kein Effekt vorliegt bezeichenen wir mit $P(H_0)$. Es allgemeiner formuliert, die Wahrscheinlichkeit für eine Studie mit einem Nulleffekt. Wenn $N$ die Gesamtzahl der betrachteten Studien bezeichnet, dann berechnet sich die Anzahl der Studien bei denen ein Nulleffekt vorliegt mittels.

\begin{equation*}
N_{H_0} = P(H_0) \cdot N
\end{equation*}

Multiplizieren wir nun diese Studien mit der Irrtumswahrscheinlichkeit $\alpha$ dann, erhalten wir wiederum diejenige Anzahl der Studien die zwar einen Nulleffekt, also eigentlich keinen Effekt haben, aber sich auf Grund der Irrtumswahrscheinlichkeit *geiirt* haben.

\begin{equation*}
N_{H_0, \text{signifikant}} = P(H_0) \cdot \alpha \cdot N
\end{equation*}

Ein einfaches Beispiel wäre, wenn $N = 200$ Studien gegeben wäre, und $P(H_0) = 0.5$ wäre, dann liegt bei $100$ Studien kein Effekt vor. Aber aus diesen $100$ Studien werden wir uns in $\alpha$ der Fälle irren, also $5$ statisisch signifikante Ergebnisse beobachten. In @fig-stats-sig-fdr-01 ist dies einmal graphisch dargestellt.
...
Die gleichen Überlegungen können für diejenigen Studien gemacht werden, bei denen tatsächlich ein Effekt vorliegt. Die Anzahl der Studien ist genau das Komplement zu denjenigen bei denen der Nulleffekt vorliegt. Formal

\begin{equation*}
N_{H_1} = (1-P(H_0))\cdot N
\end{equation*}

Die Anzahl derjenigen Studien bei denen nun ein Effekt vorliegt und die auch als statistisch signifikant beobachtet werden ist abhängig von der Power $\rho$. Dementsprechend berechnet sich die Anzahl der statistisch signifikanten Studien bei denen auch ein Effekt vorliegt mittels:

\begin{equation*}
N_{H_1,\text{signifikant}} = (1-P(H_0))\cdot \rho \cdot N
\end{equation*}

In unserem Beispiel mit $N = 200$ Studien und $P(H_0) = 0.5$ würde bei $100$ Studien ein Effekt vorliegen und bei einer Power von $\rho = 0.8$ würden $80$ dieser Studien als statistisch signifikant beobachtet werden (siehe @fig-stats-sig-fdr-02).
...
Schauen wir uns den Anteil aller signifikanten Ergebnisse an, bei denen eigentlich ein Nulleffekt vorliegt erhalten wir die false discovery rate (fdr), die folgendermaßen definiert ist (siehe @bartovs2022).
...
D.h. derjenige Anteil der statistisch signifikanten Ergebnisse bei denen es sich um einen falschen Befund handelt. D.h. es wurde ein Effekt in Form eines statistisch signifikanten Ergebnisses beobachtet obwohl eigentlich kein Effekt vorliegt. D.h. wir teilen großen $H_0$ Punkte durch die Gesamtanzahl der großen Punkte, der signifikanten Punkte (siehe @fig-stats-sig-fdr-02a).

```{r}
#| fig-cap: "Signifikante Null- und $H_1$-effekten bei $N = 200$ Studien, $P(H_0) = 0.5$ und Power $\\rho = 0.8$."
...
Die Formel \eqref{eq-stats-sig-fdr} für die fdr kann nun verwendet werden um den Zusammenhang der false discovery rate mit der Power für eine gegebene Nulleffektswahrscheinlichkeit $P(H_0)$ und $\alpha$ zu betrachten (siehe @fig-stats-sig-fdr-03).
...
In @fig-stats-sig-fdr-03 ist zu erkennen, dass die fdr besonders hoch ist, wenn die Power in der Literatur niedrig ist. D.h. wenn die mittlere Power in der Literatur durch vorwiegend kleine Studien niedrig ist, dann sinkt die Wahrscheinlichkeit, dass ein gegebenes statistisch signifikantes Ergebnis tatsächlich durch einen Effekt hervorgerufen wurde. Daher ist es für eine Forschungsdisziplin wichtig, dass die Studien ausreichend Power haben um die gewünschten Effekte tatsächlich beobachten zu können (siehe @button2013). In @fig-stats-sig-fdr-03 ist dieser Effekt besonders groß, wenn die Grundrate der tatsächlichen Effekte $1-P(H_0)$ niedrig ist bzw. eben $P(H_0)$ sehr hoch ist. Wenn nur $20\%$ der Studien tatsächlich Effekte zugrunde liegen, sind $2/3$ der Ergebnisse falsch positiv.
...
Ein interessanter Artikel welche Auswirkungen es hat, wenn Studien zu wenig Power haben @button2013. In @djulbegovic2007 ist eine interessante Diskussion darüber unter welchen Bedingungen statistisch signifikante Ergebnis als wahr angesehen werden sollten. In @borg2023 ist eine interessante Untersuchung zu der unerwartet hohen Anzahl von statistisch signifikanten Ergebnissen in der sportwissenschaftlichen Literatur dokumentiert (siehe auch @haeffel2022). In @sandercock2024 ein Beispiel, dass öfter als gedacht der Standardfehler mit der Standardabweichung verwechselt wird (siehe auch @kadlec2022)
