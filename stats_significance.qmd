...
Das Ganze auch wieder als Graph (siehe @fig-sts-coint-toss-3)
...
Schauen wir uns jetzt noch mal den dreimaligen Münzwurf an. Wenn wir kein Kopf werfen in $3$ Würfen und betrachten die Würfe wieder sequentiell, dann haben wir $\frac{1}{2}$ für die erste Zahl, $\frac{1}{2}$ für die zweite Zahl und $\frac{1}{2}$ für die dritte Zahl. Also insgesamt $P(1 \text{ Kopf}) = \frac{1}{2} \times \frac{1}{2}\times \frac{1}{2} = \frac{1}{8}$. Aber diese Wahrscheinlichkeit hat ja jedes Elementarereignis egal ob es (K,K,K) oder (K,Z,K) oder (Z,Z,K) usw. ist. Jetzt haben wir aber das Problem das wir für $1$ oder $2$ Köpfe nicht nur eine Möglichkeit haben diese Anzahl an Kopf zu beobachten. In @tbl-sts-sig-coin-toss-3 haben wir gezeigt, dass jeweils drei Möglichkeiten gibt. D.h. wir haben jetzt ein Abzählproblem. Können wir irgendwie direkt bestimmen wie viele unterschiedliche Möglichkeiten es gibt?

Schauen wir uns den Fall ein Kopf an. Wir haben ein 3-fach Tupel. Auf wie viele Arten können wir einzigartige 3-fach Tupel erzeugen mit nur einem Kopf. Nun, der Kopf ist entweder an der ersten, der zweiten oder der dritten Stelle. Das hört sich aber ähnlich an wie wie etas was wir schon vorher einmal gehört haben, als wir uns die Anzahl der möglichen Stichproben aus unserer kleinen Welt angeschaut haben. Dabei sind wir auf den Binomialkoeffizienten gestoßen @eq-binom-coef.

$$
\text{Anzahl} = \binom{n}{k} = \frac{n!}{k!(n-k)!}
$$

Wenden wir das mal auf unseren Dreifachwurf an mit $n = N = 3$ und $k = 1$

$$
\text{Kombinationen mit 1 Kopf} = \binom{3}{1} = \frac{3!}{1!(3-1)!} = \frac{3\times 2 \times 1}{1\times 2 \times 1} = 3
$$

Passt. Probieren wir das auch direkt auf das Ereignis zweimal Kopf, als mit $N = 3$ und $k = 2$ an.

$$
\text{Kombinationen mit 2 Kopf} = \binom{3}{2} = \frac{3!}{2!(3-2)!} = \frac{3\times 2 \times 1}{2\times 1 \times 1} = 3
$$

Passt auch. Wenn wir jetzt noch die beiden Definition einführen.
...
erhalten wir für die anderen beiden verbleibenden Fälle:

\begin{align*}
\text{Kombinationen mit 0 Kopf} &= \binom{3}{0} = 1 \\
\text{Kombinationen mit 3 Kopf} &= \binom{3}{3} = 1
\end{align*}

Haben wir für alle möglichen Ausgängen die Anzahl der möglichen Elementarereignisse bestimt. Allgemein erhalten wir dadurch eine Formel für die Wahrscheinlichkeiten der Ereignisse für den dreifachen Münzwurf.
