...
D.h. die Varianz $\sigma^2_Y$ von $Y$ wird mit dem Quadrat von $a$ multipliziert. Im Fall eines Zufallsvektor $\mathbf{Y}$ mit einer konstanten Matrix $\mathbf{A}$ gilt entsprechend:
...
$$
\begin{matrix}
& \underbrace{(\mathbf{X}^T\mathbf{X})^{-1}\mathbf{X}^T\mathbf{X}}_{=\mathbf{I}}\mathbf{b} &= (\mathbf{X}^T\mathbf{X})^{-1}\mathbf{X}^T\mathbf{Y} \\
\Leftrightarrow & \mathbf{b} &= (\mathbf{X}^T\mathbf{X})^{-1}\mathbf{X}^T\mathbf{Y} 
\end{matrix}
$$ {#eq-slm-matrix-bsol}

D.h. es wurde eine Lösung für $\mathbf{b}$ gefunden. Tatsächlich führt diese Matrizenlösung zu der gleichen Lösung wie diejenige die mittels der Normalengleichungen erhalten wird. Die Herleitung ohne das *Weglassen* von $\mathbf{e}$ geht eigentlich wieder über die Minimierung, ist aber hier der Einfachheit halber weggelassen worden. 
...
Die Berechnung mit `R` über `lm()` führt zu dem gleichen Ergebnis:
...
Dies sind natürlich die gleichen Werte $\hat{y}_i$ die auch über `predict()` berechnet werden.
...
Letztendlich führt `predict()` die gleiche Berechnung im Hintergrund durch.

Sei nochmal @eq-slm-matrix-bsol betrachtet um die Koeffizienten zu bestimmen:
...
::: {#exm-slm-matrix-hmatrixR}
### Berechnung von $\mathbf{H}$ in `R`

Die Berechnung von $\mathbf{H}$ für die Beispieldaten geht relativ einfach. Die Modelmatrix $\mathbf{X}$ wurde ja schon extrahiert. Dementsprechend folgt:
...
```
```{r}
X %*% solve(t(X)%*% X) %*% t(X) |> MASS::fractions()
```

Hier ist auch nochmal zu erkennen, dass die $\mathbf{H}$ eine symmetrische Matrix ist.

:::

Mit $\mathbf{H}$ lässt sich die Varianz-Kovarianz-Matrix von $\mathbf{b} = \hat{\beta}$ berechnen. Zunächst sei dazu jedoch die Varianz-Kovarianz-Matrix von $\mathbf{Y}$ betrachtet. Unter den Annahmen der einfachen linearen Regression sind die einzelnen Werte unabhängig voneinander. D.h. die Kovarianzen zwischen den $Y$-Werten sind gleich Null, formal $\text{Cov}(Y_i,Y_j) = 0, \forall i \neq j$. Weiterhin sind die Varianzen der $Y$-Werte alle gleich. D.h. es gilt $\text{Var}(Y_i) = \sigma^2$. Übertragen auf die Varianz-Kovarianz-Matrix von $\mathbf{Y}$ bedeutet dies, das auf der Hauptdiagonalen überall der Wert $\sigma^2$ steht und alle anderen Einträge gleich Null sind. Daher hat die VCV von $\mathbf{Y}$ die folgende Form:
...
Auf der rechten Seite ist nur $\mathbf{Y}$ eine Zufallsvariable und alle anderen Terme sind Konstanten. Daher gilt unter Anwendung @eq-vcv-asigma mit der Festsetzung $\mathbf{A} = \mathbf{X}(\mathbf{X}^T\mathbf{X})^{-1}$:
...
Für die zweite Zeile wurde @eq-vcv-asigma angewendet, in der dritten Zeile wurde der rechte Term entsprechend der Transponierung umsortiert. In der dritten Zeile wurde @eq-slm-matrix-varY angewendet, in der vierten Zeile die Vertauschung von Transponieren und Invertieren, da $\mathbf{X}^T\mathbf{X}$ symmetrisch ist. In der vorletzten Zeile wurde die Inverse angewendet. Letztendlich folgt, dass die Varianz-Kovarianzmatrix von $\beta$ direkt durch die Modellmatrix beeinflusst wird:
