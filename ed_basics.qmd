...
Wenn die experimentelle Einheit nach der Applikation des Treatments bzw. der Kondition aufgeteilt wird, spricht man von einer Untereinheit. Die Messungen an Untereinheiten derselben experimentellen Einheit sind in der Regel korreliert sollten daher nicht als unabhängige Ergebnisse behandelt werden.
:::

In @tbl-ed-basics-ex-01 ist ein Beispiel für eine einfaches experimentelle Design abgebildet. Es wurden insgesamt $9$ TeilnehmerInnen zufällig auf $3$ Konditionen $A_1, A_2, A_3$ verteilt und auf jeder Person wurde eine Messung unter der entsprechenden Kondition durchgeführt. Jede Person ist einzeln untersucht wurden.
...
: Beispiel für ein experientelles Design (X = Messung) {#tbl-ed-basics-ex-01}
...
In @tbl-ed-basics-ex-02 haben wir ein ähnliches Design. Im Unterschied zu @tbl-ed-basics-ex-01 sind auf jeder Person nun zwei Messungen durchgeüführt worden.
...
: Beispiel für ein experientelles Design mit zwei Messungen (X = Messung) {#tbl-ed-basics-ex-02}

Dadurch sind immernoch die $EU = 9$ und der Replikationsfaktor $=2$. Die Beobachtungseinheiten sind jetzt verdoppelt $OU = 18$ und wir erhalten einen Pseudoreplikationsfaktor von $2$.

Im nächsten Design wird es nun etwas komplizierter. Wir untersuchen drei verschiedene Treatment $A_1, A_2$ und $A_3$ mit neuen Teilnehmern. Die Teilnehmer sind aus drei verschiedenen Vereinen und wir können die Treatments immer nur einheitlich pro Verein anwenden. Die Zuordnung der Treatments an die Vereine erfolgt randomisiert.
...
: Beispiel für ein experientelles Design mit einer Gruppierungstruktur (X = Messung) {#tbl-ed-basics-ex-03}

In diesem Fall erfolgt die Zuweisung nicht auf dem Level der Teilnehmer sondern auf dem Level Vereine. Dadurch werden die Vereine die experimentellen Einheiten und $N = 3$. Dardurch haben wir keine Replikation der experimentellen Einheiten da jede Kondition nur einmal beobachtet wurde.
...
: Example for a Completely Randomized Design
...
: Example for a Completely Randomized Factorial Design with two factors
...
: Example for a Completely Randomized Block Design
...
: Example for a Split-plot Design