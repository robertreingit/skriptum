...
Hier zeigt sich ein interessantes Muster. Für mittel und schwere Handballer nimmt die Wurfgeschwindigkeit mit der Armspannweite zu. Dagegen zeigt sich der gegensätzliche Trend für leichte Handballer. D.h. der Einfluss der Armspannweite hängt davon ab wie schwer der Handballer ist. Bzw. andersherum natürlich genause. Aber dies ist die Definition eines Interaktionseffekts.
...
Im Rahmen des linearen Modells wir ein solcher Interaktionseffekt durch einen multiplikativen Term zwischen den beiden Variablen modelliert.
...
Als erstes sehen wir, dass sich die Residualvarianz $\hat{sigma}$ mehr als halbiert hat. D.h unser Modell ist schon einmal deutlich präziser geworden. Auch die Standardfehler der beiden Steigungskoeffizienten für dei Haupteffekte haben sich deutlich verringert (vgl. @tbl-mlm-inter-handball-mod2). Dementsprechend sind beide Koeffizienten statistisch signifikant. Der neue dazugekommene Interaktionseffekt ist ebenfalls statistisch signifikant mit einem sehr kleinen Standardfehler. Jetzt müssen wir nur noch verstehen was der Koeffizient für $\beta_3$ eigentlich bedeutet.

Schauen wir dazu zunächst die Vorhersagen $\hat{y}_i$ unter den beiden Modellen Modell 1 und Modell 2 an. Beide Modell arbeiten mit zentrierten Prädiktorvariablen. Vereinfachen wir die Daten und nehmen nur drei verschiedene Werte für die Armspannweite $as = \{-5,0,5\}$ D.h. einen hypothetischen Handballer mit der Durchschnittsspannweite, sowie jeweils einen mit $5$ cm kürzerer bzw. $5$ cm längerer Spannweite. Wir kombinieren diese drei Werte mit fünf verschiedenen Körpermassen $bm = \{-6,-3,0,3,6\}$. D.h. eine durchschnittliche Körpermasse und jeweils $3$ und $6$ Kg darüber und darunter. Das führt dann insgesamt zu dieser Kombination von Prädiktorvariablen (siehe @tbl-mlm-inter-handball-xpred).

```{r}
#| label: tbl-mlm-inter-handball-xpred
#| tbl-cap: "Alle Kombinationen von Prädiktorvariablen."
  
expand.grid(as = c(-3,0,3), bm = c(-6,-3,0,3,6)) |> 
...
In @fig-mlm-inter-simple-1 sehen wir, dass die vorhergesagten Werte $\hat{y}_i$ für verschiedene Körpermassen jeweils nur parallel verschoben sind. Da die unterschiede zwischen den Körpermassen jeweils $3$ Kg unterscheiden sich für einen gegebenen Wert der Armspannweite die Werte konstant zwischen den Linien um den Faktor $\beta_{\text{body mass}} \times 3$. Dies sieht komplett anders unter Modell 3 aus (siehe @fig-mlm-inter-simple-2). Hier sehen wir, dass die Unterschiede zwischen den verschiedenen Armspannweiten davon abhängen wie schwer die Handballer ist. Während bei kleinen Armspannweiten die Unterschiede sehr klein sind, werden die Unterschiede zwischen den Gewichtskategorien umso größer je größer die Armspannweite wird. Es handelt sich eben um einen Interkationseffekt.

Der Interaktionseffekt ist dabei symmetrisch und hängt nicht davon ab, welche Variable als $x$-Variable in dem Graphen verwendet wird. In @fig-mlm-inter-simple-02 sind beide Möglichkeiten einmal nebeneinander dargestellt.
...
In beiden Fällen beobachten wir den Interaktionseffekt.

Als nächstes Versuchen wir zu verstehen warum die Multiplikation im Model Sinn macht und was die Koeffizienten in @tbl-mlm-inter-handball-mod3 bedeuten. In @fig-mlm-inter-mod3-coefs-1 sind noch einmal die Steigungen für die Körpermasse in Abhängigkeit von der Armspannweite abgebildet. Der Einfachheit halber nur drei Stück für die Armspannweiten $as = \{-10,0,10\}$cm.
...
#|   - "3D Streudiagramm des Interaktionsmodells und gefittet Ebene unter Model 2"
...
Zurückkommend auf unser Ausgangsbeispiel. Wie hat sich die Hinzunahme des Interaktionseffekts eigentlich auf den Modellfit ausgewirkt. Vergleichen wir die beiden Residuenplots dazu.
...
Der gleiche Vergleich noch einmal für die qq-Plots der Residuen.
...
### Zuschlag^*^
Schauen wir uns noch einmal an, was passiert mit dem Koeffizienten im Interaktionsmodell wenn die Prädiktorvariablen $x_{ki}$s zentriert werden?
...
Wir sehen, dass nur der Interaktionsterm gleich ist unter dem zentrierten und dem unzentrierten Modell. Dagegen unterscheiden sich die Koeffizienten der Haupteffekte deutlich zwischen den beiden Modellen voneinander. 
