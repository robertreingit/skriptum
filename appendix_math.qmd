...
k \cdot (s\cdot\mathbf{A}) &= (k\cdot s) \cdot \mathbf{A} \\
(k + s)\cdot \mathbf{A} &= k\cdot \mathbf{A} + s\cdot\mathbf{A}
\end{aligned}
$$
...
$$
\text{Spur} = \mathrm{Tr}(\mathbf{A}) = \sum_{i=1}^n a_{ii}
$$ {#eq-matrix-trace}

::: {#exm-math-app-mattrace}

Sei zum Beispiel die Matrix $\mathbf{A}$ gegeben:
...
Aus einer beliebigen Matrix kann eine symmetrische Matrix gewonnen werden indem die Matrix mit ihrer Transponierten entweder Vor- oder Nachmultipliziert wird. Sei zum Beispiel die Matrix $\mathbf{S}$ gegeben mit:
...
Dies lässt sich auch direkt unter Anwendung der Regeln @eq-matrix-transpose und @eq-matrix-transtrans.
