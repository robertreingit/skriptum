...
Beginnen wir mit einem einfachen Beispiel. Es wurde ein Datensatz gesammelt, bei dem zwei Gruppen miteinander verglichen wurden. Die eine Gruppe bezeichnen wir als Treatmentgruppe (TRT) und hat beispielsweise eine spezielle Krafttraining durchgeführt, während wir die andere Gruppe als Kontrollgruppe (CON) bezeichnen die ein *übliches* Krafttraining durchgeführt hat. Beide Gruppen sind gleich groß gewesen und bestehen jeweils aus $N_i = 20, i = 1,2$ Personen. Die Teilnehmerinnen und Teilnehmer sind auch zufällig in die beiden Gruppen eingeteilt worden, so dass keine Unterschied zwischen den beiden Gruppen vor Beginn des Trainings bestand. Es wurde nach der Trainingsperiode das folgende Ergebnis erhalten (siehe @fig-why_stats). 
...
Die Antwort werden wir in den folgenden Abschnitt erarbeiten. Im Zuge dessen werden wir die notwendigen Werkzeuge entwickeln um die verschiendenen einer statistischen Analyse von Daten zugrundeliegen Schritte zu verstehen und anwenden zu können. Dabei lässt sich leider die Mathematik und die damit verbundene Abstraktion nicht vollständig umgehen wird aber auf eine Mindesmaß reduziert.
