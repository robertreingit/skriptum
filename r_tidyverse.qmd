...
Eine Variante von `mutate()` ist `transmute()` welches nur die neu erstellten Spalten beibehält und *alle* anderen Spalten ausschließt.

```{r}
#| echo: true

df |> transmute(cmj_m = cmj / 100)
```
...
Die folgenden Abschnitte behandeln ein paar Spezialthemen, die im *normalen Tagesgeschäft* eher selten auftreten, aber es dann manchmal ganz praktisch ist die verschiedenen Möglichkeiten schon mal gesehen zu haben um sich bei Bedarf den Ansatz nochmal gezielt nachschauen zu können.
...
Manchmal ist es notwendig die Werte einer Variablen aus dem tibble zu extrahieren. D.h. es soll eine Spalte zu einem *normalen* `R` Vektor gemacht werden. Dies kann mit dem Befehl `pull()` durchgeführt werden. Sollen zum Beispiel nur die Einträge für das Alter extrahiert werden, kann der folgende Befehl benutzen werden:
...
`pull()` liefert einen Vektor mit den Einträgen aus der Spalte `age`. Wie immer bleibt das ursprüngliche tibble dabei unverändert.

#### Gruppieren innnerhalb von `mutate()`

In der `mutate()`-Funktion gibt es auch einen Parameter `.by` der erlaubt innerhalb von `mutate()` Gruppen zu bilden und über diese Gruppen Werte zu berechnen (Achtung: vor `by` ist ein Punkt `.`). Soll zum Beispiel die Abweichung der Sprunghöhen für Frauen und Männer vom jeweiligen Mittelwert berechnet werden und dabei eben nicht über Gruppen zusammengefasst werden, dann kann diese mit `.by` folgendermaßen ausgedrückt werden.

```{r}
#| echo: true

df |> mutate(delta_cmj = cmj - mean(cmj), .by = gender)
```

D.h. diese Abweichungen sind nicht die Abweichungen vom Gesamtmittelwert,

```{r}
#| echo: true

df |> mutate(delta_cmj = cmj - mean(cmj))
```

Sondern die Abweichungen von den jeweiligen Gruppenmittelwerten. Mit `group_by()` könnte das gleiche erreicht werden, ist aber etwas umständlicher.

```{r}
#| echo: true

df |> group_by(gender) |> mutate(delta_cmj = cmj - mean(cmj)) |> ungroup()
```

#### Zeilenweise Berechnungen mit `rowwise()`

Manchmal kommt es vor, dass man eine Berechnung über mehrere Spalten hinweg für jede Zeile durchführen muss, aber nicht `pivot_longer()` verwenden möchte. Sei zum Beispiel der folgende Datensatz gegeben:
...
Die Aufgabe nun besteht darin, für jede Zeile den Mittelwert der `vo2`s zu berechnen. Der direkte Ansatz mit `mutate()` führt zu:

```{r}
#| echo: true

df_row |> mutate(
  vo2_bar = mean(vo2_1 + vo2_2 + vo2_3 + vo2_4 + vo2_5 + vo2_6)
)
```

Bei der `rowwise()` wird die Funktion der Verben im tidyverse verändert.
...
df_row |>
  rowwise() |> 
  mutate(vo2_bar = mean(c(vo2_1, vo2_2, vo2_3, vo2_4, vo2_5, vo2_6)))
```

Hier mussten die Spalten nicht mehr addiert werden, sondern die Art und Weise wie `mean()` arbeitet ist jetzt entlang der Zeilen. Dies unterscheidet sich von der normalen Ausführung. Hier nochmal zum Vergleich.
...
Hier wurde ein Mittelwert über alle Spalte berechnet. Die Anwendung von `rowwise()` ist vor allem in Verbindung mit `c_across()` praktisch, da `c_across()` die **tidy select** Mechanismen erlaubt und könnte im Beispiel wie folgt angewendet werden.
...
df_row |> 
  rowwise() |> 
  mutate(vo2_bar = mean(c_across(vo2_1:vo2_6)))
```

Im Ausdruck ist zu sehen, das das `tibble` noch die Eigenschaft `# Rowwise:` besitzt. Um wieder den normalen Bearbeitungsmodus einzusetzen muss noch ein `ungroup()` angewendet werden.
...
Wie schon angesprochen kommen die Spezialthemen eher nur selten zum Einsatz und die Kenntnis von `filter()`, `select()`, `mutate()`, `summarize()`, `group_by()` reicht für den überwiegenden Teil der Datenverarbeitungsaufgaben die im Alltag auftreten. Es gilt wie immer, erst die wiederholte Anwendung der Methoden erleichtert den Umgang, daher sollte man sich nicht am Anfang davon demotivieren lassen, dass alles noch etwas unübersichtlich erscheint.
