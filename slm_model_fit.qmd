...
Dazu schauen wir uns zunächst noch einmal an, was überhaupt Residuen $e_i$ sind. Dazu gehen wir noch mal von den grundlegenden Modellannahmen aus (siehe Formel \eqref{eq-sim-model-lr}).
...
Wir gehen von einem linearen Zusammenhang in den Koeffizienten zwischen der Variablen $x_i$ und den Variablen $y_i$. Additiv dazu kommt ein normalverteilte Fehler $\epsilon_i$ die im Mittel den Wert $\mu = 0$ und eine Standardabweichung von $\sigma$. Dies führt dazu,  dass $y_i$ für jeden gegebenen Wert von $x_i$ einer Normalverteilung mit $\mathcal{N}(\beta_0 + beta_1 x_i, \sigma^2)$ folgen. Dies führt zu der bereits bekannten graphischen Darstellung (siehe @fig-slm-model-resid-1).
...
Für jeden gegebenen Wert von $X$ sind die $Y$-Werte Normalverteilt. Die Varianz dieser Normalverteilungen ist gleich, $\sigma$, während der Mittelwert $\mu$ immer um den Wert der Regressionsgeraden verschoben ist.

Eine Möglichkeit die Residuen $\epsilon_i$ abschätzen ist die Abweichungen der beobachteten Werten $y_i$ von den vorhergesagten Werten $\hat{y}_i$ auf der Regressionsgeraden zu bestimmen (siehe Formel \eqref{eq-sim-model-res-1}).
...
Diese Abweichungen können als Schätzer für die wahren Residuen $\epsilon_i$ verwendet werden.
...
Daraus ergibt sich auch eine erste Möglichkeit die Modellannahmen graphisch zu überprüfen indem die Residuen $e_i$ gegen die vorhergesagten Werte $\hat{y}_i$ abgetragen werden. In `R` können die Residuen mittels der Funktion `residuals()` bzw. der Kurzform `resid()` ermittelt werden. Als Parameter wird das gefittete `lm()`-Objekt übergeben.
...
Die Vorhergesagten Werte $\hat{y_i}$ erhält man mit der Funktion `predict()` mit der wir uns im nächsten Kapitel noch ausführlich auseinandersetzen. Als Parameter wird wiederum das gefittete Modell übergeben.
...
Mit Hilfe dieser beiden Variablen kann nun ein Residuenplot erstellt werden (siehe @fig-sim-model-rplot-1).
...
Der Plot sollte jetzt am Besten so aussehen, dass die Residuen $e_i$ gleichmäßig sich oberhalb und unterhalb um die Nulllinie verteilen und keine weiteren Strukturen oder Muster in Bezug auf $\hat{y}_i$ zu erkennen sind.

Problem im Residuenplot könnten Beispielsweise wie in den folgenden beiden Diagrammen auftreten (siehe @fig-sim-model-rplot-2)
...
qq-Plot ist die Kurzform von Quantile-Quantile-Plot. D.h. es werden die Quantilen von zwei Variablen gegeneinander abgetragen. Um die Funktionsweise besser zu verstehen schauen wir uns erst einmal ein Spielzeugbeispiel an. In @tbl-sim-model-toy-1 ist eine kleiner Datensatz mit $n = 5$ Datenpunkten angezeigt.
...
Wir wollen jetzt überprüfen ob dieser Datensatz einer Normalverteilung folgt (Wohlwissend das mit fünf Datenpunkten keine Verteilungsannahme überprüft werden kann). Dazu schauen wir uns auch noch einmal die wohlbekannte Standardnormalverteilung an (siehe @fig-slm-model-toy-1).
...
Im nächsten Schritt unterteilen wir die Standardnormalverteilung $\Phi(x)$ in $n+1 = 6$ Abschnitt, die alle die gleiche Wahrscheinlichkeit (=Fläche unter der Dichtefunktion haben) und bestimmen die jeweiligen Trennpunkte, die Quantilen.
...
Als Nächstes sortieren wir unsere Datenpunkt (@tbl-slm-model-toy-1) aufsteigend nach der Größe (siehe @tbl-slm-model-toy-2). Diese Werte werden jetzt als empirische Quantilen bezeichnet. Unter der Annahme, dass die Werte eine repräsentative Stichprobe aus der Verteilung darstellen, würden wir erwarten, dass wenn wir weiter Werte ziehen würden, etwa $\frac{1}{6}$ der Werte kleiner als der kleinste Wert wären, $\frac{2}{6}$ der weiteren Werte kleiner als der 2. kleinste Wert wären und so weiter und so fort.
...
Die Idee hinter dem qq-Plot ist daher, die empirischen Quantilen gegen die theoretischen Quantilen abzutragen (siehe @fig-slm-model-toy-3). Wenn die beobachteten Werte der theoretischen Verteilung folgen, dann sollte der Graph nahezu einer Geraden folgen.
...
In @fig-slm-model-toy ist es tatsächlich schwierig eine Gerade zu erkennen was aber auch vor allem damit zusammenhäng, dass bei $n=5$ eine realistische Verteilungsannahme nicht überprüft werden kann. Wie gesagt, dies diente nur um die Erstellung zu verstehen.
