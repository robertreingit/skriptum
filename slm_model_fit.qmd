...
Als Nächstes sortieren wir unsere beobachteten Datenpunkte aus @tbl-slm-model-toy-1 aufsteigend nach der Größe (siehe @tbl-slm-model-toy-2). Diese Werte konnen als *empirische* Quantilen bezeichnet. Unter der Annahme, dass die Werte eine repräsentative Stichprobe aus der Verteilung darstellen, erwarten wir, dass wenn wir weiter Werte ziehen würden, etwa $\frac{1}{6}$ der Werte kleiner als der kleinste Wert wären, $\frac{2}{6}$ der weiteren Werte kleiner als der 2. kleinste Wert wären und so weiter und so fort.
...
Die Idee hinter dem qq-Plot besteht nun darin, die empirischen Quantilen gegen die theoretischen Quantilen abzutragen (siehe @fig-slm-model-toy-3). Wenn die beobachteten Werte der theoretischen Verteilung folgen, dann sollte der resultierende Graph nahezu einer Geraden folgen.
...
In `R` gibt es zwei einfache Methoden qq-Plot zu erstellen. Mittels des Standardgrafiksystem können mit den Funktionen `qqnorm()` und `qqline()` qq-Plots mit der dazugehörigen Gerade erstellt werden. Für das `ggplot()`-System stehen die `geom`s `geom_qq()` und `geom_qq_line()` zur Verfügung. Wichtig ist hierbei, das in `aes()` der Parameter `sample` belegt werden muss. Für unser Spielzeugbeispiel sieht dies folgendermaßen aus:
