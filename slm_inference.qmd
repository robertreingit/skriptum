...
In `R` kann der Wert $\hat{\sigma}$ über die Funktion `sigma()` aus dem gefitteten `lm()`-Modell extrahiert werden.
...
An Formel \eqref{eq-slm-inf-beta0-se} ist zu erkennen, dass wenn die $X$-Werte den Mittelwert $0$ haben, dass $\sigma^2[\beta_0]$ gleich dem Standardfehler für den Mittelwert SEM wird. Was auch wiederum Sinn macht, da in diesem Fall $\beta_0 = \bar{y}$ gilt.

Dies führt zu den beiden Hypothesen für $\beta_0$:
...
Noch einmal zu erwähnen ist, dass die beiden Parameter $\hat{\beta}_0$ und $\hat{\beta}_1$ welche die Regressionsgerade beschreiben, Schätzer für die Parameter aus einer Population sind der die beiden Parameter $\beta_0$ und $\beta_1$ den zugrundeliegenden Zusammenhang zwischen den beiden Variablen beschreiben. Diese Betrachtung ist parallel zu derjenigen, wenn wir z.B. anhand des Mittelwerts $\bar{x}$ den wahren Populationsmittelwert $\mu$ versuchen zu schätzen. D.h. wir haben eine Populationsregressionsgerade, die wir mit Hilfe der Daten versuchen zu schätzen. Die *wahre* Regressionsgerade werden wir aber niemals mit 100%-iger Sicherheit bestimmen, eben genausowenig wie wir den Populationsmittelwert $\mu$ nicht mittels $\bar{x}$ bestimmen können.

## Weitere Hilfsfunktionen für `lm()` in `R`

Wenn einmal direkt die Standardfehler verwendet werden können, können diese mittels der Funktion `vcov()` bestimmt werden.
...
`vcov()` gibt die Varianz-Kovarianz Matrize für die Modellparameter aus (Achtung eine andere Matrize als oben im Zusammenhang mit dem Gauss-Markov-Modell!). Die Standardfehler sind dann entsprechend den Parameter die Wurzel der Werte auf der Hauptdiagonalen.
...
Entsprechend könnten wir die Konfidenzintervalle auch von-Hand ausreichnen. Zum Beispiel das Konfidenzintervall für $\hat{\beta_1}$.
