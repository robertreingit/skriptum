...
Der Wert wird auch bei der `summary()`-Funktion ausgegeben.
...
### Der Determinationskoeffizient $R^2$ als Maß für die Modellgüte
...
In @fig-slm-pred-simpledev sind die drei verschiedenen Abweichung noch einmal graphisch dargestellt.
...
Wir sehen anhand der Pfeile in @fig-slm-pred-simpledev das ein additiver Zusammenhang zwischen den Abweichungen besteht. Der blaue Pfeil setzt sich aus der Addition zwischen dem grünen und dem grauen Pfeil zusammen. Rein algebraisch gilt ebenfalls:
...
Aus diesem Zusammenhang lässt sich eine Metrik erstellen um die Modellgüte zu beurteilen. Schauen wir uns dazu an, was passiert wenn wir einen perfekten Zusammenhang zwischen der $X$ und $Y$-Variablen haben. In @fig-slm-pred-r2-perfect ist ein solcher Zusammenhang abgebildet.
...
In @fig-slm-pred-r2-null ist das gegenteilige Beispiel abgebildet. Es besteht keine Zusammenhang zwischen der $x$ und $y$-Variable.
...
D.h. es wurde ein sehr guter Modellfit bezüglich eines linearen Zusammenhang zwischen der Anlaufgeschwindigkeit und der Sprungweite mit $R^2 = `r round(summary(mod)[['r.squared']], 2)`$ gefunden. Hier haben wir aber auch direkt den Fall, dass wir ein hohes $R^2$ beobachten aber die absolute Varianz trotzdem relativ hoch ist.
